...
// FORK (declared in cfg.libOverrides): Astryx story files call
// `stylex.create(...)` directly at module scope (a common demo pattern in
// this DS's stories) — StyleX requires that call to be compiled away by its
// babel/esbuild plugin; left uncompiled it throws "Unexpected 'stylex.create'
// call at runtime" the moment the story module loads, which is why every
// component previously rendered "root empty" (the whole story module threw
// before any component ever mounted, not a missing-provider issue). Adding
// the esbuild-target StyleX plugin here transforms it during the same
// esbuild pass the converter already runs for story compilation.
import stylexEsbuildPlugin from '@stylexjs/unplugin/esbuild';
...
export function storyImportPlugins({ PKG, GLOBAL, extraEntries = [], exported, cfg, pkgDir }) {
  ...
  return {
    plugins: [
      stylexEsbuildPlugin({
        unstable_moduleResolution: { type: 'commonJS', rootDir: pkgDir ? resolve(pkgDir, '..', '..') : process.cwd() },
      }),
      dsShim,
      storybookStubPlugin(),
      consoleStub,
